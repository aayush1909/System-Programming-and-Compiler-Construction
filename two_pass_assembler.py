      ...
      for n2, j in enumerate(val[2]):
         if j in [x[0] for x in symbol_t]:
            ...
            if j[: j.index('(')] in [x[0] for x in symbol_t]:
               intermediate_t[n1][2][n2] = str([x[0] for x in symbol_t].index(j[: j.index('(')])) + ' ST'          # for st with index
            else:
               intermediate_t[n1][2][n2] = str([x[0] for x in literal_t].index(j[ : j.index('(')])) + ' I'         # for lit_t with index
   ...
   if flag != 1:
      ...
      if flag1 != 1:                                  # both digit/equ
         ...
         if len(mc_table[n1][1]) > 1:
            mc_table[n1][1] = str(mc_table[n1][1][0]) + str(', ') + str(int(mc_table[n1][1][1]) - int(base_t[0][0])) + str(' (') + str(index) + str(', ') + str(base_t[0][1]) + str(' )')
print('\nMachine code table')
for _ in mc_table:
   print(_)
